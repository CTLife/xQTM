...
my $input_g    = '';  ## such as "6_rawBAM/PE/3_STAR"
my $output_g   = '';  ## such as "7_finalBAM/PE/3_STAR"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 7: Only the mapped reads with MAPQ>20 are retained and reads on other chromosomes, all of them are removed. 
        ...
        Usage:
               perl xRNA-Seq_7.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]   
        For instance:
               nohup  time  perl xRNA-Seq_7.pl   -genome hg38   -in 6_rawBAM/PE/3_STAR   -out 7_finalBAM/PE/3_STAR       > xRNA-Seq_7.PE.3_STAR.runLog 2>&1  &    
...
## Version Infromation
my $version = "    The 7th Step, version 1.2,  2023-07-25.";
...
$input_g  = '6_rawBAM/PE/3_STAR';           ## This is only an initialization value or suggesting value, not default value.
$output_g = '7_finalBAM/PE/3_STAR';         ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  xRNA-Seq_7.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("samtools  view   -h    --threads $numCores_g    -q 9   -L $genome_g.genome.chromsize.selected.bed   -o $output_g/$temp.bam   $input_g/$temp.bam    >> $output2_g/$temp.runLog     2>&1  "); 
