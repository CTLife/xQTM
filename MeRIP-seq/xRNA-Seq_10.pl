...
my $output_g = '';  ## such as "10_rawCounts/PE/3_STAR"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------ 
        Usage:
               perl  xRNA-Seq_10.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl  xRNA-Seq_10.pl    -in 7_finalBAM/PE/3_STAR   -out 10_rawCounts/PE/3_STAR    > xRNA-Seq_10.1.runLog  2>&1   &
...
## Version Infromation
my $version = "    The 10th Step, version 1.2,  2023-07-25. ";
...
$output_g = '10_rawCounts/PE/3_STAR';        ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  xRNA-Seq_10.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#bamfiles_g; $i++) {
    ...
    system("featureCounts    -p     -O  --minOverlap 5   -M    -s 2     -T $numCores_g  -F GTF   -a $GTF_file_g    -o $output_g/featureCounts/$temp.featureCounts  $input_g/$bamfiles_g[$i]    > $output_g/$temp.featureCounts.runLog.txt  2>&1 ");
