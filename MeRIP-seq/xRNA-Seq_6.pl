...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 6: Mapping reads to the reference genome by using 6 tools (mappers or aligners): 
                Kallisto, Salmon, STAR, HISAT2, BWA (for RNA spike-in), and Bowtie2 (for RNA spike-in)
        ...
        Usage:
               perl  xRNA-Seq_6.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  [-mis mismatch] 
        For instance:
               nohup time  perl  xRNA-Seq_6.pl   -genome hg38   -in 3_removedDups   -out 6_rawBAM/PE  -mis 0.05    > xRNA-Seq_6.PE.runLog  2>&1  &
               nohup time  perl  xRNA-Seq_6.pl   -genome hg38   -in 4_SE-RC         -out 6_rawBAM/SE  -mis 0.05    > xRNA-Seq_6.SE.runLog  2>&1  &
...
## Version Infromation
my $version = "   The 6th Step, version 1.2,  2023-07-25.";
...
$output_g = '6_rawBAM';             ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  xRNA-Seq_6.pl  -help' \n";
    exit 0;
}
...
my  $HISAT2_index_g    = "$commonPath_g/hisat-3n/RefGenomes/4N_HFM/$genome_g/$genome_g";
my  $BWA_index_g       = "$commonPath_g/BWA/RefGenomes/ERCC92/ERCC92";
my  $Bowtie2_index_g   = "$commonPath_g/bowtie2/RefGenomes/ERCC92/ERCC92";
...
say   "BWA:       $BWA_index_g";       ## (for RNA spike-in)
say   "Bowtie2:   $Bowtie2_index_g";   ## (for RNA spike-in)
###################################################################################################################################################################################################
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("kallisto quant  --threads=$numCores_g   --single    -l 150    -s 100  --verbose  --index=$Kallisto_index_g    --output-dir=$Kallisto_1_g/$temp    $input_g/$singleEnd_g[$i]    >>  $Kallisto_1_g/$temp.runLog  2>&1");
}
...
## by default, STAR performs local (not end-to-end) alignment.  --alignEndsType 
###################################################################################################################################################################################################
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g              --outSAMtype BAM SortedByCoordinate  --quantMode GeneCounts  --alignSJDBoverhangMin 2   --outSAMmultNmax 1   --readFilesCommand zcat           --outFilterMultimapNmax  100    --outFilterMismatchNmax 999   --outFilterMismatchNoverLmax 0.5   --outFilterMismatchNoverReadLmax  $mismatch_g    --outFilterScoreMinOverLread 0.2   --outFilterMatchNmin 20  --outFilterMatchNminOverLread 0.1           --outFileNamePrefix  $STAR_3_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$end1  $input_g/$end2   >> $STAR_3_g/$temp.runLog  2>&1 ");                                                                 
        system("rename  s/Aligned.sortedByCoord.out.bam/bam/   $STAR_3_g/*Aligned.sortedByCoord.out.bam");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,  $end2\n";
        ##system("hisat2         --threads $numCores_g   -q   --phred33                                --sp 1,0      --trim5 1  --trim3 1    --summary-file    $HISAT2_4_g/1.un.$temp.summary   -x $HISAT2_index_g    -1 $input_g/$end1        -2 $input_g/$end2     -S $HISAT2_4_g/$temp.sam    >> $HISAT2_4_g/1.un.$temp.runLog  2>&1");
        ##system("hisat2         --threads $numCores_g   -q   --phred33       --rna-strandness FR      --sp 1,0      --trim5 1  --trim3 1    --summary-file    $HISAT2_4_g/2.F.$temp.summary    -x $HISAT2_index_g    -1 $input_g/$end1        -2 $input_g/$end2     -S $HISAT2_4_g/$temp.sam     >> $HISAT2_4_g/2.F.$temp.runLog  2>&1");
        ##system("hisat2         --threads $numCores_g   -q   --phred33       --rna-strandness RF      --sp 1,0      --trim5 1  --trim3 1    --summary-file    $HISAT2_4_g/3.R.$temp.summary    -x $HISAT2_index_g    -1 $input_g/$end1        -2 $input_g/$end2     -S $HISAT2_4_g/$temp.sam     >> $HISAT2_4_g/3.R.$temp.runLog  2>&1");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        ##system("hisat2        --threads $numCores_g   -q   --phred33                             --sp 1,0      --trim5 1  --trim3 1  --summary-file  $HISAT2_4_g/1.un.$temp.summary   -x $HISAT2_index_g    -U $input_g/$singleEnd_g[$i]        -S $HISAT2_4_g/$temp.sam    >> $HISAT2_4_g/1.un.$temp.runLog  2>&1");
        ##system("hisat2        --threads $numCores_g   -q   --phred33        --rna-strandness F   --sp 1,0      --trim5 1  --trim3 1  --summary-file  $HISAT2_4_g/2.F.$temp.summary    -x $HISAT2_index_g    -U $input_g/$singleEnd_g[$i]        -S $HISAT2_4_g/$temp.sam     >> $HISAT2_4_g/2.F.$temp.runLog  2>&1");
        ##system("hisat2        --threads $numCores_g   -q   --phred33        --rna-strandness R   --sp 1,0      --trim5 1  --trim3 1  --summary-file  $HISAT2_4_g/3.R.$temp.summary    -x $HISAT2_index_g    -U $input_g/$singleEnd_g[$i]        -S $HISAT2_4_g/$temp.sam     >> $HISAT2_4_g/3.R.$temp.runLog  2>&1");
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("bwa mem  -t $numCores_g     -L 1,1    -T 20    $BWA_index_g   $input_g/$end1  $input_g/$end2    > $BWA_g/$temp.sam");
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("bwa mem  -t $numCores_g   -L 1,1   -T 20     $BWA_index_g   $input_g/$singleEnd_g[$i]   > $BWA_g/$temp.sam"); 
}
