...
my $HELP = '
        ...
        Usage:
               perl  xRNA-Seq_13_callPeaks_MeRIP.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]   [-ip  IPdir]  [-out outDir]
        For instance:
               nohup time  perl  xRNA-Seq_13_callPeaks_MeRIP.pl   -genome hg38   -in Input   -ip  IP -out MACS2    > xRNA-Seq_13_callPeaks_MeRIP.runLog.txt   2>&1    &
        ----------------------------------------------------------------------------------------------------------
        Optional arguments:
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  xRNA-Seq_13_callPeaks_MeRIP.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#BAMfiles_g; $i++ ) { 
    ...
    system("macs2  callpeak   --control  $input_g/$ctrl    --treatment $IP_g/$temp.bam       --format BAM   --gsize $genome_size_g   --keep-dup 10      --outdir $outputDir3/$temp    --name $temp      --verbose 3         --qvalue 0.05     --nomodel         >> $outputDir3/$temp.runLog    2>&1");                    
    system("macs2  callpeak   --control  $input_g/$ctrl    --treatment $IP_g/$temp.bam       --format BAM   --gsize $genome_size_g   --keep-dup 5       --outdir $outputDir4/$temp    --name $temp      --verbose 3         --qvalue 0.05     --nomodel         >> $outputDir4/$temp.runLog    2>&1");    
    system("macs2  callpeak   --control  $input_g/$ctrl    --treatment $IP_g/$temp.bam       --format BAM   --gsize $genome_size_g   --keep-dup 1       --outdir $outputDir5/$temp    --name $temp      --verbose 3         --qvalue 0.05      -nomodel         >> $outputDir5/$temp.runLog    2>&1");   
