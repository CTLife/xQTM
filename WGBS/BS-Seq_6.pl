...
my $output_g = '';  ## such as "6_callMethylation"
...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Welcome to use BSDA (BS-Seq Data Analyzer).
        ...
        Usage:
               perl  BS-Seq_6.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]
        For instance:
               perl  BS-Seq_6.pl   -genome hg38   -in 5_finalBAM   -out 6_callMethylation    > BS-Seq_6.runLog
...
$output_g = '6_callMethylation';   ## This is only an initialization value or suggesting value, not default value.
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    my $align_report = "6_rawBAM/PE/2A_Bismark/$temp/$temp" . "_PE_report.txt";
