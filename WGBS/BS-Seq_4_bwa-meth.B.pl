...
my $HELP = '
        ...
        Usage:
               perl  BS-Seq_4_bwa-meth.B.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  [-mis mismatch] 
        For instance:
               nohup time  perl  BS-Seq_4_bwa-meth.B.pl   -genome hg38   -in 3_removedDups   -out 4_rawBAM_bwa-meth/PE  -mis 0.05    > BS-Seq_4_bwa-meth.B.PE.runLog  2>&1  &
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  BS-Seq_4_bwa-meth.B.pl  -help' \n";
    exit 0;
}
...
###################################################################################################################################################################################################
my $bismark2_g     = "$output_g/2_Genome";
&myMakeDir($bismark2_g);

{ ## Start bismark
say   "\n\n\n\n\n\n##################################################################################################";
say   "Mapping reads to the reference genome by using bwameth ......";
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system(" bwameth.py    --threads $numCores_g      --reference $Bismark_index_g     $input_g/$end1     $input_g/$end2    >  $bismark2_g/$temp.sam  ");
        system(" samtools  sort  -m 2G  -o $bismark2_g/$temp.bam   --output-fmt bam  -T $bismark2_g/ypTemp_$temp   --threads $numCores_g    $bismark2_g/$temp.sam   ");
        system(" samtools  index           $bismark2_g/$temp.bam   ");
        system(" rm  $bismark2_g/$temp.sam " );
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("  bwameth.py    --threads $numCores_g      --reference $Bismark_index_g     $input_g/$singleEnd_g[$i]    >  $bismark2_g/$temp.sam  ");
        system("samtools  sort  -m 2G  -o $bismark2_g/$temp.bam   --output-fmt bam  -T $bismark2_g/ypTemp_$temp   --threads $numCores_g    $bismark2_g/$temp.sam   ");
        system("samtools  index           $bismark2_g/$temp.bam   ");
        system( "rm  $bismark2_g/$temp.sam " );
}
...
&myMakeDir("$output2_g/2_Bismark");
system( "multiqc    --title bismark        --verbose  --export   --outdir $output2_g/2_Bismark                    $bismark2_g        >> $output2_g/2_Bismark/MultiQC.1.runLog    2>&1" );
