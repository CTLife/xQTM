...
my $HELP = '
        ...
        Step 2: Remove adapters and bases with low quality by using Trimmomatic and/or trim_galore. 
                ...
                by using 6 tools: FastQC, fastp, FastQ_Screen, MultiQC, prinseq, and bbcountunique.sh.
     
                If this script works well, you do not need to check the the versions of the tools or packages which are used in this script. 
                ...
                The versions of softwares and packages are used in this script (This is shown for reference only. And do not need to exactly match them.): 
                        ...
                        bbmap,         39.01   (bbmap.sh --version)
                        Trimmomatic,   0.39    (trimmomatic.jar   -version)
                        trim_galore,   0.6.10  (trim_galore   --version)
                        cutadapt,      4.2     (cutadapt      --version)
       
        Usage:
               perl BS-Seq_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl BS-Seq_2.pl   -in 1_rawFASTQ   -out 2_removedAdapters    > BS-Seq_2.runLog.txt  2>&1    &
...
## Version Infromation
my $version = "    The 2nd Step, version 0.9.9, 2023-09-20.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl BS-Seq_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        $temp =~ s/\.fastq\.gz$//   ;
        $temp =~ s/\.fastq$//    ;
        system(  "java  -jar  $Trimmomatic_g  PE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt      $input_g/$end1  $input_g/$end2    $output_g/$end1              $output_g/unpaired-$end1                $output_g/$end2              $output_g/unpaired-$end2                  ILLUMINACLIP:$Trimmomatic2_g:2:30:10:2:TRUE   SLIDINGWINDOW:4:15  MINLEN:20    >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog  2>&1");                                                   
        #system("  java  -jar  $Trimmomatic_g  PE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt      $input_g/$end1  $input_g/$end2    $output_g/Trimmomatic/$end1  $output_g/Trimmomatic/unpaired-$end1    $output_g/Trimmomatic/$end2  $output_g/Trimmomatic/unpaired-$end2      ILLUMINACLIP:$Trimmomatic2_g:2:30:10:2:TRUE   SLIDINGWINDOW:4:15  MINLEN:15    >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog  2>&1");                                                   
        #system("trim_galore  --quality 20   --phred33  --length 15    --clip_R1 2   --clip_R2 2    --output_dir  $output_g    --paired  $output_g/Trimmomatic/$end1  $output_g/Trimmomatic/$end2          >> $output2_g/$temp.trim_galore.runLog  2>&1"); 
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        $temp =~ s/\.fastq\.\S+$//   ;
        $temp =~ s/\.fastq$//    ;
        system("java  -jar  $Trimmomatic_g  SE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt   $input_g/$singleEnd_g[$i]  $output_g/$singleEnd_g[$i]                ILLUMINACLIP:$Trimmomatic2_g:2:30:10:2   SLIDINGWINDOW:4:15  MINLEN:15      >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog.txt  2>&1");                    
        #system("java  -jar  $Trimmomatic_g  SE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt   $input_g/$singleEnd_g[$i]  $output_g/Trimmomatic/$singleEnd_g[$i]    ILLUMINACLIP:$Trimmomatic2_g:2:30:10:2   SLIDINGWINDOW:4:15  MINLEN:15      >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog.txt  2>&1");                    
        #system("trim_galore  --quality 20   --phred33  --length 15      --clip_R1 2  --output_dir  $output_g       $output_g/Trimmomatic/$singleEnd_g[$i]      >> $output2_g/$temp.trim_galore.runLog  2>&1");   
...
## --three_prime_clip_R1 <int>     Instructs Trim Galore to remove <int> bp from the 3' end of read 1 (or single-end reads) AFTER adapter/quality trimming has been performed.
