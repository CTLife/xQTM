...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 5: Mapping reads to the reference genome by using 6 tools (mappers or aligners): 
        ...
        Usage:
               perl  BS-Seq_4A.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  [-mis mismatch] 
        For instance:
               nohup time  perl  BS-Seq_4A.pl   -genome hg38   -in 3_removedDups   -out 4_rawBAM/PE  -mis 0.05    > BS-Seq_4A.PE.runLog  2>&1  &
...
## Version Infromation
my $version = "   The 4th Step, version 1.2,  2023-08-30.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  BS-Seq_4A.pl  -help' \n";
    exit 0;
}
...
my  $Lambda_index_g    = "$commonPath_g/Bismark/RefGenomes/lambda";
my  $Bismark_index_g   = "$commonPath_g/Bismark/RefGenomes/$genome_g";
...
sub  myQC_BAM_1  {
    ...
    for ( my $i=0; $i<=$#Files; $i++ ) {
        ...
        say    "\t......$temp";
        $temp =~ s/\.bam$//  ||  die;
        system( "mv   $dir1/$temp.bam     $QCresults/$temp.bam" );
        system("samtools  sort  -m 2G  -o $dir1/$temp.bam   --output-fmt bam  -T $dir1/ypTemp_$temp   --threads $numCores_g    $QCresults/$temp.bam    >> $SAMtools/$temp.runLog    2>&1");
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system("bismark    --maxins 1000  --unmapped   --output_dir $lambda_2_g/$temp     --bam  --basename $temp   --bowtie2  -p $numCores_g       $Lambda_index_g    -1 $input_g/$end1    -2 $input_g/$end2    >  $lambda_2_g/$temp.runLog   2>&1");
        system("mv   $lambda_2_g/$temp/*.bam     $lambda_2_g/$temp.bam" );  
        
        #my $unmapped1 = "$lambda_2_g/$temp/$temp"."_unmapped_reads_1.fq.gz"; 
        #my $unmapped2 = "$lambda_2_g/$temp/$temp"."_unmapped_reads_2.fq.gz";         
        #system("bismark       --output_dir  $lambda_SE_2_g             --bam  --basename $temp.R1   --bowtie2  -p $numCores_g        $Lambda_index_g     $unmapped1     >  $lambda_SE_2_g/$end1.runLog   2>&1");
        #system("bismark       --output_dir  $lambda_SE_2_g   --pbat    --bam  --basename $temp.R2   --bowtie2  -p $numCores_g        $Lambda_index_g     $unmapped2     >  $lambda_SE_2_g/$end2.runLog   2>&1");
        #system("rm  $unmapped1");
        #system("rm  $unmapped2");
}
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("bismark   --output_dir  $lambda_2_g/$temp    --bam  --basename $temp   --bowtie2  -p $numCores_g       $Lambda_index_g     $input_g/$singleEnd_g[$i]      >  $lambda_2_g/$temp.runLog   2>&1");
        system("mv   $lambda_2_g/$temp/*.bam     $lambda_2_g/$temp.bam" );  
}

} ## End bismark

&myMakeDir("$output2_g/1A_ToLambda");
&myMakeDir("$output2_g/1B_ToLambda_unmapped_SE");
system( "multiqc    --title bismark        --verbose  --export   --outdir $output2_g/1A_ToLambda                    $lambda_2_g        >> $output2_g/1A_ToLambda/MultiQC.1.runLog    2>&1" );
system( "multiqc    --title bismark        --verbose  --export   --outdir $output2_g/1B_ToLambda_unmapped_SE        $lambda_SE_2_g     >> $output2_g/1B_ToLambda_unmapped_SE/MultiQC.2.runLog    2>&1" );
